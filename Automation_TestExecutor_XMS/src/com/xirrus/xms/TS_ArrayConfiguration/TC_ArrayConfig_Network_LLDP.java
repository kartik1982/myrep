...
@Test
public class TC_ArrayConfig_Network_LLDP extends TestSuiteBase{
	//CDP Paramters
	...
	@Test(priority=1)
	public void testCase_VerifyArrayConfigNetworkLLDPParamters() throws InterruptedException, IOException{
		App_Log.debug("Start-Verify Array Config Network LLDP Paramters");
		//*****************************************************************
		...
		if(xmslocallib.isRadioButtonSelected("Enable LLDP",1))lldpStateGUI="enabled";else lldpStateGUI="disabled";
		//Verify Admin Contact name configured into array by GUI
		App_Log.debug("GUI-Verification:-Verify Network LLDP value is enabled in array");
		boolean guilldpstate=xmslocallib.verifyStringequals("LLDPState", this.getClass().getSimpleName(), methodName, lldpStateGUI, lldpStateExp);
		//Verify Admin Contact name get Configured into array by SNMP
		App_Log.debug("SNMP-Verification:-Verify Network LLDP value is enabled in array");
		lldpStateSNMP=xmslocallib.getValueBySNMP(arrayIPadd, communityStr, lldpEnableIOD);
		boolean snmplldpstate=xmslocallib.verifyStringequals("LLDPState", this.getClass().getSimpleName(), methodName, lldpStateSNMP, "1");
		//Verify Admin Contact name get pushed into array by CLI
		App_Log.debug("CLI-Verification:-Verify Network LLDP value is enabled in array");
		lldpStateCLI=xmslocallib.getValuebyCLI_LLDP(arrayIPadd, "State");
		boolean clilldpstate=xmslocallib.verifyStringequals("LLDPState", this.getClass().getSimpleName(), methodName, lldpStateCLI, lldpStateExp);
	
		App_Log.debug("End-Verify Array Config Network LLDP Paramters");		
	}
